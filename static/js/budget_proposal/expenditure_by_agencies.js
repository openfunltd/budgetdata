$(document).ready(function() {
  //歲出機關別預算表
  var expenditureByAgenciesTable = $('#expenditure-by-agencies-table').DataTable({
    pageLength: 50,
    columnDefs: [
      {
        targets: 0,
    ...
    initComplete: function() {
      ...
      $('#expenditure-by-agencies-table thead th').eq(0).find('span.dt-column-order').addClass('d-none');
    }
  });

  $('#expenditure-by-agencies-table tbody').on('click', 'td.details-control', function() {
    var td = $(this);
    //don't provide action when description data
    if (td.hasClass('no-description')) { return; }

    var tr = $(this).closest('tr');
    var row = expenditureByAgenciesTable.row(tr);
